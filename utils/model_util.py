...
class FlowNetS(nn.Module):
    def __init__(self, input_channels = 2, batchNorm=True):
        ...
        self.deconv4 = deconv(770,256)
        self.deconv3 = deconv(386,128)
        self.deconv2 = deconv(194,64)

        self.predict_flow6 = predict_flow(512)
        self.predict_flow5 = predict_flow(770)
        self.predict_flow4 = predict_flow(386)
        self.predict_flow3 = predict_flow(194)
        self.predict_flow2 = predict_flow(66+input_channels)
    ...
    def forward(self, x):
        ...
        flow6       = self.predict_flow6(out_conv4)
        flow6_up    = self.upsampled_flow6_to_5(flow6)
        out_deconv5 = self.deconv5(out_conv4)
        # import ipdb;ipdb.set_trace()
        
        concat5 = torch.cat((out_conv3,out_deconv5,flow6_up),1)
        flow5       = self.predict_flow5(concat5)
        flow5_up    = self.upsampled_flow5_to_4(flow5)
        out_deconv4 = self.deconv4(concat5)
        
        concat4 = torch.cat((out_conv2,out_deconv4,flow5_up),1)
        flow4       = self.predict_flow4(concat4)
        flow4_up    = self.upsampled_flow4_to_3(flow4)
        out_deconv3 = self.deconv3(concat4)
        
        concat3 = torch.cat((out_conv1,out_deconv3,flow4_up),1)
        flow3       = self.predict_flow3(concat3)
        flow3_up    = self.upsampled_flow3_to_2(flow3)
        out_deconv2 = self.deconv2(concat3)

        concat2 = torch.cat((x,out_deconv2,flow3_up),1)
